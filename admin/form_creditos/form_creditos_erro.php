...
$old_error_handler = set_error_handler("form_creditos_scriptcase_error_handler");

function form_creditos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['form_creditos']['contr_erro']) && $_SESSION['scriptcase']['form_creditos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_creditos->contr_form_creditos->Apl_com_erro = true;
            if ($inicial_form_creditos->contr_form_creditos->NM_ajax_flag)
            {
                $inicial_form_creditos->contr_form_creditos->NM_ajax_info['errList']['geral_form_creditos'][] = $msg;
                return;
            }
            form_creditos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_creditos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_creditos'])  && '' != $_SESSION['scriptcase']['error_icon']['form_creditos'])  ? $_SESSION['scriptcase']['error_icon']['form_creditos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_creditos']) && '' != $_SESSION['scriptcase']['error_close']['form_creditos']) ? $_SESSION['scriptcase']['error_close']['form_creditos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_creditos']) && '' != $_SESSION['scriptcase']['error_span_title']['form_creditos']) ? $_SESSION['scriptcase']['error_span_title']['form_creditos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_creditos']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_creditos']) ? $_SESSION['scriptcase']['error_icon_title']['form_creditos'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_creditos']['error_buffer'] .= ob_get_flush();
}
